...
class TabAccessor:
    ...
    def last_focused_progress_percent(self) -> str:
        ...
        if not tab or not tab.last_focused_window_with_progress_id:
            return ''
        w = get_boss().window_id_map.get(
            tab.last_focused_window_with_progress_id
        )
        if w is None:
            return ''
...
def apply_title_template(
    draw_data: DrawData, tab: TabBarData, index: int, max_title_length: int = 0
) -> str:
    if tab.tab_id < 0:
        return (
            tab.title
        )  # synthetic tab — render title literally, skip user template
...
DrawTabFunc = Callable[
    [DrawData, Screen, TabBarData, int, int, int, bool, ExtraData], int
]
...
class CustomDrawTitleFunc:
    def __init__(
        self,
        data: dict[str, Any],
        implementation: Callable[[dict[str, Any]], str] | None = None,
    ):
...
class TabExtent(NamedTuple):
    ...
    def shifted(self, shift: int) -> 'TabExtent':
        return TabExtent(
            self.tab_id,
            CellRange(
                self.cell_range.start + shift, self.cell_range.end + shift
            ),
        )
...
class TabBar:
    ...
    def update_blank_rects(
        self, central: Region, tab_bar: Region, vw: int, vh: int
    ) -> None:
        opts = get_options()
        blank_rects: list[Border] = []
        bg = (
            BorderColor.tab_bar_margin_color
            if opts.tab_bar_margin_color is not None
            else BorderColor.default_bg
        )
        if opts.tab_bar_margin_height:
            if opts.tab_bar_edge == BOTTOM_EDGE:
                ...
                if opts.tab_bar_margin_height.inner:
                    blank_rects.append(
                        Border(0, central.bottom, vw, tab_bar.top, bg)
                    )
            else:   # top
                if opts.tab_bar_margin_height.outer:
                ...
                if opts.tab_bar_margin_height.inner:
                    blank_rects.append(
                        Border(0, tab_bar.bottom, vw, central.top, bg)
                    )
        g = self.window_geometry
        left_bg = right_bg = bg
        if (
            opts.tab_bar_margin_color is None
            and (opacity := background_opacity_of(self.os_window_id))
            is not None
            and opacity >= 1
        ):
    ...
    def layout(self) -> None:
        ...
        available_width_for_left_margin = max(
            0, tab_bar.width - self.margin_width - cell_area_width
        )
        extra_width = max(
            0, tab_bar.width - 2 * self.margin_width - cell_area_width
        )
        left_margin = min(
            self.margin_width + extra_width // 2,
            available_width_for_left_margin,
        )
        self.laid_out_once = True
        self.window_geometry = g = WindowGeometry(
            left_margin,
            tab_bar.top,
            left_margin + cell_area_width,
            tab_bar.bottom,
            s.columns,
            s.lines,
        )
    ...
    def update(self, data: Sequence[TabBarData]) -> None:
        ...
        def draw_tab(
            ...
            cell_ranges.append(
                TabExtent(tab_id=tab.tab_id, cell_range=CellRange(before, end))
            )
            if (
                not ed.for_layout
                and t is not last_tab
                and s.cursor.x > s.columns - max_tab_lengths[i + 1]
            ):
    ...
    def align_with_factor(self, factor: int = 1) -> None:
        ...
        if end < self.screen.columns - 1:
            ...
            self.tab_extents = tuple(
                te.shifted(shift) for te in self.tab_extents
            )
